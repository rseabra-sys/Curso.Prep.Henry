...
function buscarAmigo(amigos, nombre) {
  ...
  if (i < amigos.length) {
     ...
     return 'No existe en el arreglo amigos con el nombre: ' + nombre;
  }
}
...
function sumArray(array, n) {
  ...
  for (var i = 0; i < array.length; i++) {
    var valorArray1 = array[i];

    for (var j = 0; j < array.length; j++) {
      var valorArray2 = array[j];
    
      if (valorArray1 === valorArray2) continue;

      xsuma = valorArray1 + valorArray2;
      if (xsuma === n) {
	  	  return true;
   	  } else {
	  	  return false;
...
function pluck(array, propiedad) {
  ...
  // Tu código acá:
     ...
     var valor = productos.map(pluck);

     function pluck(propiedad) {
